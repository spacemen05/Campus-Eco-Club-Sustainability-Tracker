...
$volunteerName = $stmt->fetchColumn() ?: "Volunteer";

// Total eco points
$stmt = $pdo->prepare("SELECT COALESCE(SUM(points),0) FROM recycling_logs WHERE volunteer_id = ?");
$stmt->execute([$volunteerId]);
$ecoPoints = (int)$stmt->fetchColumn();

// Total recycling logs
$stmt = $pdo->prepare("SELECT COUNT(*) FROM recycling_logs WHERE volunteer_id = ?");
$stmt->execute([$volunteerId]);
  ...
  <div class="panel border rounded-3xl p-5 shadow-sm lift">
    <div class="flex items-center justify-between gap-4">
      <div>
        ...
        <div class="text-2xl font-extrabold tracking-tight"><?= htmlspecialchars($volunteerName) ?></div>
        <div class="text-sm text-slate-600 mt-1">Live data from MySQL </div>
      </div>

      <div class="hidden sm:flex gap-2">
        <a class="btn px-4 py-2 rounded-2xl bg-slate-900 text-white hover:opacity-90" href="volunteer_events.php">Browse Events</a>
        <a class="btn px-4 py-2 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700" href="volunteer_recycling_submit.php">Submit Log</a>
      </div>
    </div>
  </div>
</div>
...
<section class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
  <div class="panel border rounded-3xl p-5 shadow-sm lift">
    <div class="flex items-start justify-between">
      ...
      <div class="w-11 h-11 rounded-2xl bg-emerald-100 text-emerald-800 flex items-center justify-center text-xl"></div>
    </div>
    <div class="mt-4">
      <a class="btn inline-flex px-4 py-2 rounded-2xl border bg-white/70 hover:bg-white" href="volunteer_points.php">View points</a>
    </div>
  </div>
  ...
  <div class="panel border rounded-3xl p-5 shadow-sm lift">
    <div class="flex items-start justify-between">
      ...
      <div class="w-11 h-11 rounded-2xl bg-sky-100 text-sky-800 flex items-center justify-center text-xl"></div>
    </div>
    <div class="mt-4">
      <a class="btn inline-flex px-4 py-2 rounded-2xl bg-slate-900 text-white hover:opacity-90" href="volunteer_events.php">Go to events</a>
    </div>
  </div>
  ...
  <div class="panel border rounded-3xl p-5 shadow-sm lift">
    ...
    <div class="mt-4">
      <a class="btn inline-flex px-4 py-2 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700" href="volunteer_recycling_submit.php">Submit log</a>
    </div>
  </div>
</section>
...
<section class="panel border rounded-3xl shadow-sm overflow-hidden">
  <div class="p-5 flex items-center justify-between">
    ...
    <a class="btn text-sm px-3 py-2 rounded-2xl border bg-white/70 hover:bg-white" href="volunteer_events.php">View all</a>
  </div>
    ...
    <tr class="trow">
      ...
      <td class="px-5 py-3 text-right">
        <a class="btn px-3 py-2 rounded-2xl border bg-white/70 hover:bg-white"
           href="volunteer_event_view.php?id=<?= (int)$e['id'] ?>">View</a>
        <a class="btn px-3 py-2 rounded-2xl bg-emerald-600 text-white hover:bg-emerald-700"
           href="volunteer_event_signup.php?id=<?= (int)$e['id'] ?>">Sign Up</a>
      </td>
    </tr>
  <?php endforeach; ?>
<?php endif; ?>
