...
function uploadProofImage(array $file): array {
  ...
  // Save to: eco_tracker/public/uploads/recycling_proofs/
  $targetDir = __DIR__ . "/../uploads/recycling_proofs";
  if (!is_dir($targetDir)) {
    mkdir($targetDir, 0777, true);
  }

  $safeName = "proof_s{$GLOBALS['studentId']}_" . date("Ymd_His") . "_" . bin2hex(random_bytes(4)) . "." . $ext;
  ...
  // Store relative path (relative to /public)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($category === "" || $quantity === "") {
    ...
    // Upload image
    [$ok, $pathOrError] = uploadProofImage($_FILES['proof'] ?? []);
    if (!$ok) {
      ...
      try {
        ...
        $stmt = $pdo->prepare("
          INSERT INTO recycling_logs (student_id, category, quantity, points, proof_path)
          VALUES (?, ?, ?, ?, ?)
        ");
        $stmt->execute([$studentId, $category, $quantity, $points, $pathOrError]);
  ...
  <?php if ($successMsg): ?>
    <div class="mb-4 p-4 rounded-2xl bg-emerald-50 text-emerald-800 border border-emerald-100">
      ...
      <div class="mt-2 flex gap-4">
        <a class="underline" href="student_points.php">View Eco Points</a>
        <a class="underline" href="student_dashboard.php">Back to Dashboard</a>
      </div>
    </div>
  <?php elseif ($errorMsg): ?>
